...
def ncwrite (ncfile):
    ...
    max_temp = numpy.array(ncfile.variables['max_temp'][:],dtype=type(ncfile.variables))
    ...
    for time_index in dt:
        for lt in lt_rng:
            for ln in ln_rng:
                print("Max Temp for:", dt_range_perfile[time_index])
                #print(lat[lt],lon[ln],dailyrain[time_index,lt,ln])
                dict_row = {'Time': str(dt_range_perfile[time_index]), 'Latitude' : lat[lt], 'Longitude' : lon[ln], 'MeasureName': 'MaxTemperature', 'MeasureValue' : max_temp[time_index][lt][ln]}
...
my_bucket = s3.Bucket('s3://silo-open-data/annual/max_temp/')

ncfiles = my_bucket.objects.all()
print(ncfiles)

s3_ds = Dataset('s3://silo-open-data/annual/max_temp/'+yearType+'.max_temp.nc', 'r', format='NETCDF4')
ncwrite(s3_ds )
